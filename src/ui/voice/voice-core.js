...
const STATE_CFG = {
  idle:        { amp: 0.003, spd: 0.10, r: [76,108,112],  g: [82,116,112],  b: [82,96,92]    },
  listening:   { amp: 0.055, spd: 0.75, r: [168,226,232], g: [190,238,228], b: [188,218,210] },
  recognizing: { amp: 0.55,  spd: 4.50, r: [86,188,198],  g: [176,228,218], b: [214,176,96]  },
  done:        { amp: 0.10,  spd: 1.20, r: [92,186,150],  g: [170,224,186], b: [122,160,118] },
  processing:  { amp: 0.15,  spd: 1.10, r: [132,214,196], g: [174,226,208], b: [206,176,110] },
  error:       { amp: 0.10,  spd: 0.70, r: [232,126,88],  g: [126,76,54],   b: [48,38,34]    },
  event:       { amp: 0.60,  spd: 4.00, r: [245,202,104], g: [205,154,74],  b: [88,76,56]    },
  speaking:    { amp: 0.09,  spd: 1.00, r: [142,224,232], g: [188,230,218], b: [226,178,96]  },
};
...
function getCloudWsUrl() {
  const apiBase = window.__JARVIS_API_BASE__ || '';
  if (apiBase) {
    try {
      const url = new URL('/voice/cloud', apiBase);
      url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:';
      return url.href;
    } catch {}
  }
  const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
  return `${protocol}//${window.location.host}/voice/cloud`;
}
const VOICE_PROVIDER_KEY = 'jarvis-voice-provider';
const VOICE_MIC_DEVICE_KEY = 'jarvis-voice-mic-device-id';
const MIC_ACTIVITY_FLOOR = 0.006;
const TTS_RESUME_PCM_GUARD_MS = 700;
const TTS_RESUME_TRANSCRIPT_GUARD_MS = 1400;

function makeCloudAsrConfig(getLang) {
  const provider = window.__JARVIS_VOICE_PROVIDER__ || localStorage.getItem(VOICE_PROVIDER_KEY) || 'local';
  const mode = window.__JARVIS_INTERFACE_MODE__ === 'standby' ? 'wake' : 'conversation';
  // Wake phrases are commonly spoken in English while the normal conversation is Chinese.
  // Ask Paraformer for both hypotheses during wake mode; conversation keeps the selected UI language.
  const lang = mode === 'wake' ? 'zh,en' : (getLang?.()?.split('-')[0] || 'zh');
  return { type: 'config', provider, lang, mode };
}
...
export function createVoiceCore({ canvas, transcript, getChatInput, getSendMessage, getLang }) {
  ...
  function drawFrame(now) {
    ...
    if (micData) {
      ...
      lastVol = vol;
      if (vol > diagPeakVol) diagPeakVol = vol;
      ...
      try { window.dispatchEvent(new CustomEvent('jarvis:voice-level', { detail: { level: vol, ttsLevel: ttsVol, status: sk } })); } catch {}
  ...
  // ─── 采集诊断（定位长语音丢字根因；localStorage 'jarvis-voice-diag'='0' 关闭，默认开） ───
  const DIAG_ON = localStorage.getItem('jarvis-voice-diag') !== '0';
  let diagTimer = null;
  ...
  let diagTotalChunks = 0, diagTotalBytes = 0, diagPeakVol = 0, diagLastCloudEvent = '', diagLastError = '';
  let diagSessionStartedAt = 0;
  ...
  function resetVoiceDiagnostics() {
    diagChunks = 0; diagBytes = 0; diagMaxGapMs = 0; diagLastChunkTs = 0;
    diagTotalChunks = 0; diagTotalBytes = 0; diagPeakVol = 0;
    diagTranscripts = 0; diagLastTranscriptTs = 0;
    diagLastCloudEvent = ''; diagLastError = '';
    diagSessionStartedAt = Date.now();
  }
  function diagStart() {
  ...
  // 也就修了「说到几十个字球变绿后不再出字」。localStorage 'jarvis-voice-watchdog'='0' 可关。
  const WATCHDOG_ON = localStorage.getItem('jarvis-voice-watchdog') !== '0';
  const STALL_RECONNECT_MS = 3500;   // 仍在说却这么久没转录 → 判定停滞
  const WATCHDOG_SPEECH_VOL = 0.016;  // 判定「人在说话」的音量阈值（与 continuous SPEECH_VOL 同量级）
  ...
  function handleAsrMessage(msg) {
    if (msg.type === 'transcript') {
      ...
      if (Date.now() < dropTranscriptUntil) {
        resetTranscriptAccumulation();
        lastTranscriptText = '';
        pendingInterim = '';
        if (transcript) transcript.textContent = '';
      ...
      try {
        window.dispatchEvent(new CustomEvent('jarvis:voice-transcript', {
          detail: {
            text: msg.text || '',
            accumulated: lastTranscriptText || '',
            final: !!msg.is_final,
            changed: !!isFinal,
            seg: msg.seg ?? null,
          }
  ...
  function connectCloudWs() {
    ...
    const ws = new WebSocket(getCloudWsUrl());
    ws.binaryType = 'arraybuffer';
    cloudWs = ws;

    ws.onopen = () => {
      if (cloudWs !== ws) return;
      ws.send(JSON.stringify(makeCloudAsrConfig(getLang)));
      setStatus('listening');
    ...
    ws.onerror = (error) => {
      if (cloudWs === ws) {
        diagLastError = error?.message || 'ASR WebSocket error';
        setStatus('error');
      }
    };
    ...
    ws.onclose = (event) => {
      ...
      if (!cloudWsIntentional && micActive) {
        ...
        diagReconnects++;
        diag('ws-closed → reconnect in 800ms', 'reconnects=' + diagReconnects + ' code=' + (event?.code || 0) + ' reason=' + (event?.reason || ''));
  ...
  async function startCloudStream(stream, generation) {
    ...
    if (!micActive || generation !== sessionGeneration) {
      stopCloudStream();
      return false;
    }
  ...
  function handlePcmChunk(i16) {
    ...
    if (Date.now() < dropPcmUntil) return;
    if (bargeinBuffering) {
  ...
  async function startSession() {
    ...
    syncState();
    const existingTrack = micData?.stream?.getAudioTracks?.()[0];
    const stream = existingTrack?.readyState === 'live' ? micData.stream : await startMic();
    if (stream && (generation !== sessionGeneration || !micActive)) {
      stream.getTracks().forEach(track => track.stop());
      if (micData?.stream === stream) {
        try { micData.actx?.close(); } catch {}
        micData = null;
      }
      return null;
    ...
    const started = await startCloudStream(stream, generation);
    if (!started) return null;
    return stream;
  }

  // 停止会话（= 原 stopVoiceInput 的 core 部分）。模式自有的计时器/标志由 onSessionStop 清。
  function stopSession({ keepIntent = false, keepMonitor = false } = {}) {
    sessionGeneration += 1;
    ...
    reconnectBuffer = [];
    micActive = false;
    monitorActive = !!(keepMonitor && micData);
    if (!keepIntent) userWantedMic = false;
    bargeinBuffer = [];
    bargeinBuffering = false;
    stopCloudStream();
    if (!monitorActive) stopMic();
    diagStop();
    stopWatchdog();
    setStatus('idle');
    if (transcript) transcript.textContent = '';
  ...
  async function startMonitor() {
    if (micData?.stream?.getAudioTracks?.()[0]?.readyState === 'live') {
      micActive = false;
      monitorActive = true;
      userWantedMic = false;
      suspendedByMedia = false;
    ...
    if (!stream || generation !== sessionGeneration) return false;
    micActive = false;
    monitorActive = true;
    userWantedMic = false;
    suspendedByMedia = false;
  ...
  async function resumeSession(fromBargein = false) {
    ...
    const bufferedChunks = fromBargein ? bargeinBuffer.slice() : [];
    bargeinBuffer = [];
    bargeinBuffering = false;
    if (!fromBargein) {
      const now = Date.now();
      dropPcmUntil = now + TTS_RESUME_PCM_GUARD_MS;
      dropTranscriptUntil = now + TTS_RESUME_TRANSCRIPT_GUARD_MS;
      reconnectBuffer = [];
    }
    ...
    if (micActive && micData && (cloudWorkletNode || cloudProcessor)) {
      ...
      const bargeinWs = new WebSocket(getCloudWsUrl());
      bargeinWs.binaryType = 'arraybuffer';
      cloudWs = bargeinWs;
      bargeinWs.onopen = () => {
        if (cloudWs !== bargeinWs) return;
        bargeinWs.send(JSON.stringify(makeCloudAsrConfig(getLang)));
      ...
      bargeinWs.onerror = (error) => {
        if (cloudWs === bargeinWs) {
          diagLastError = error?.message || 'ASR WebSocket error';
          setStatus('error');
        }
      };
  ...
  function getDiagnostics() {
    const track = micData?.stream?.getAudioTracks?.()[0] || null;
    return {
      captureMode: diagCaptureMode,
      chunks: diagTotalChunks,
      bytes: diagTotalBytes,
      transcripts: diagTranscripts,
      peakVol: Number(diagPeakVol.toFixed(4)),
      lastVol: Number(lastVol.toFixed(4)),
      micActive,
      monitorActive,
      userWantedMic,
      suspendedByMedia,
      ...
      micMuted: Boolean(track?.muted),
      micSettings: track?.getSettings?.() || {},
      cloudReadyState: cloudWs?.readyState ?? null,
      lastCloudEvent: diagLastCloudEvent,
      lastError: diagLastError,
      startedAt: diagSessionStartedAt,
      durationMs: diagSessionStartedAt ? Date.now() - diagSessionStartedAt : 0,
      activityFloor: MIC_ACTIVITY_FLOOR,
    };
  }

  function clearSelectedMicDevice() {
    localStorage.removeItem(VOICE_MIC_DEVICE_KEY);
  }
  ...
  return {
    ...
    resetTranscriptAccumulation,
    getDiagnostics,
    clearSelectedMicDevice,
    // 运行时状态访问
    get micActive() { return micActive; },
    get monitorActive() { return monitorActive; },
