...
④ 密钥填写错误或账户欠费 — 检查控制台报错信息`,
      },
      {
        ...
        content: `贾维斯首选 ASR 方案，阿里云百炼实时语音识别，中文效果出色，延迟低。
...
文档：https://www.xfyun.cn/doc/asr/rtasr/API.html`,
  ...
  voice_tts: {
    ...
    icon: '🔊',
    summary: '语音合成将文字转为 Agent 语音输出。默认使用本地 Jarvis Piper 声线（jarvis-high）和金属化处理；云端 TTS（豆包、MiniMax、OpenAI、ElevenLabs、火山引擎）是可选扩展。配置入口：点击左上角 ⚙ → 语音设置 → TTS。',
    sections: [
      {
        title: '为什么 Agent 说话没有声音？',
        content: `常见原因：
① 本地 Jarvis 声线模型缺失 — 请运行 npm run voice:install:jarvis
② 误切到了云端 TTS 但未配置密钥 — 需在语音设置中填写对应服务商的 API Key
③ 密钥失效或账户欠费 — 检查服务商控制台余额和密钥状态
④ 输出设备被系统路由到虚拟声卡或已拔掉的耳机 — 在语音输出设置中切回真实扬声器/耳机`,
      },
      {
        title: '本地 Jarvis 声线（默认）',
        content: `贾维斯默认 TTS 方案是本地 Piper/VITS Jarvis 英音模型，不需要在线 TTS API Key。

配置字段（POST /settings/tts）：
■ ttsProvider = "jarvis"
■ ttsVoiceId = "jarvis-high"

安装：
1. 在项目目录运行 npm run voice:install:jarvis
2. 确认 models/jarvis/en/en_GB/jarvis/high/jarvis-high.onnx 存在
3. 使用 npm run probe:jarvis-tts 验证本地合成

说明：如果本地模型失败，Jarvis 会报错，不会自动切到系统女声。`,
      },
      {
        title: '豆包语音合成 2.0（首选推荐）',
        content: `贾维斯首选 TTS 方案，字节跳动豆包语音合成 2.0，中文音色丰富，流式输出，延迟低。
...
→ ttsProvider — 服务商（jarvis/doubao/minimax/openai/elevenlabs/volcano/system）
→ ttsVoiceId — 音色 ID
