...
const USER_AGENT = 'JarvisLocalAgent/1.0 (+https://localhost)'
...
function platformSearchUrl(platform, title) {
  ...
  if (!query) return ''
  switch (platform) {
    case 'douyin': return `https://www.douyin.com/search/${query}`
    case 'xiaohongshu': return `https://www.xiaohongshu.com/search_result?keyword=${query}`
    case 'wechat': return `https://weixin.sogou.com/weixin?type=2&query=${query}`
    case 'weibo': return `https://s.weibo.com/weibo?q=${query}`
    default: return ''
  }
}
...
function normalizeItems(platform, rawItems, source) {
  ...
  return list
    .map((item, idx) => {
      ...
      const directUrl = String(item?.url || item?.share_url || item?.link || item?.jump_url || '').trim()
      return {
        platform,
        ...
        url: directUrl || platformSearchUrl(platform, title),
        urlKind: directUrl ? 'direct' : 'search',
        source,
      }
    })
    .filter(Boolean)
