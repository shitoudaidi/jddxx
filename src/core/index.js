...
import { collectGeoWeather, getGeoWeatherBlock, getGeoWeatherMeta } from './geo-weather.js'
...
let startupEnvironmentRefreshStarted = false

async function refreshStartupEnvironment() {
  if (startupEnvironmentRefreshStarted) return
  startupEnvironmentRefreshStarted = true
  console.log('[startup] Background environment refresh starting')

  try {
    await collectSystemInfo()
  } catch (err) {
    console.warn('[startup] system-info refresh failed:', err?.message || err)
  }

  try {
    collectDesktopInfo(getDesktopPath())
  } catch (err) {
    console.warn('[startup] desktop scan failed:', err?.message || err)
  }

  try {
    collectInstalledSoftware()
  } catch (err) {
    console.warn('[startup] installed-software scan failed:', err?.message || err)
  }

  try {
    collectLocalResources()
  } catch (err) {
    console.warn('[startup] local-resources scan failed:', err?.message || err)
  }

  let geoResult = null
  try {
    geoResult = await collectGeoWeather()
  } catch (err) {
    console.warn('[startup] geo-weather refresh failed:', err?.message || err)
  }

  try {
    await collectTrending(geoResult?.location?.country_code)
  } catch (err) {
    console.warn('[startup] trending refresh failed:', err?.message || err)
  }

  try {
    await collectAgents()
  } catch (err) {
    console.warn('[startup] agent scan failed:', err?.message || err)
  }

  try {
    await loadInstalledTools()
  } catch (err) {
    console.warn('[startup] installed tools load failed:', err?.message || err)
  }
  ...
  try {
    ...
    console.warn('[startup] skills refresh failed:', err?.message || err)
  }

  emitEvent('startup_environment_ready', { timestamp: nowTimestamp() })
  console.log('[startup] Background environment refresh complete')
}

// Scan the user's desktop (shortcuts cached by mtime, regular files scanned every time)
// Environment context is refreshed after the API is online.
...
// Installed extension tools are loaded by the background environment refresh.

// Load Agent Skills metadata. Full SKILL.md bodies are injected only when a turn matches.
// Agent skills are refreshed by the background environment refresh.
...
const RUN_TURN_WATCHDOG_MS = Math.max(30_000, Number(
  process.env.JARVIS_RUN_TURN_WATCHDOG_MS || process.env.JARVIS_RUN_TURN_WATCHDOG_MS || 90_000
) || 90_000)
const IDLE_MONITOR_INTERVAL_MS = 60_000
...
function shouldSkipStartupSelfCheck() {
  const skip = String(
    process.env.JARVIS_SKIP_STARTUP_SELF_CHECK || process.env.JARVIS_SKIP_STARTUP_SELF_CHECK || ''
  ).trim()
  if (/^(1|true|yes|on)$/i.test(skip)) return true

  const run = String(
    process.env.JARVIS_RUN_STARTUP_SELF_CHECK || process.env.JARVIS_RUN_STARTUP_SELF_CHECK || ''
  ).trim()
  return !/^(1|true|yes|on)$/i.test(run)
}
...
function shouldRunAwakeningTicks() {
  return /^(1|true|yes|on)$/i.test(String(
    process.env.JARVIS_RUN_AWAKENING_TICKS || process.env.JARVIS_RUN_AWAKENING_TICKS || ''
  ).trim())
}

function shouldRunAutonomousTicks() {
  return /^(1|true|yes|on)$/i.test(String(
    process.env.JARVIS_RUN_AUTONOMOUS_TICKS || process.env.JARVIS_RUN_AUTONOMOUS_TICKS || ''
  ).trim())
}

function getAwakeningTicks() {
  const raw = getConfig(AWAKENING_CONFIG_KEY)
  if (raw === null || raw === undefined || raw === '') return shouldRunAwakeningTicks() ? 10 : 0
...
// Jarvis 工作台兼容：把线索状态派生成"栈视图"（后台按活跃时间升序 + 前台垫底=栈顶），
...
function buildStartupSelfCheckDirections(checkState) {
  ...
  return [
    ...
    `2. Call speak text="正在检查联网能力"; call ui_show("SelfCheckStepCard", {step:2, total:3, name:"联网能力"}) and save the returned id as step_card_id. Then call web_search once for "current time Shanghai" and verify the tool returns a non-empty result. Record the result and call ui_hide(step_card_id).`,
    `3. Call speak text="正在检查工作台结果通道"; call ui_show("SelfCheckStepCard", {step:3, total:3, name:"结果通道"}) and save the returned id as step_card_id. Call ui_update on that id with {name:"结果通道已连接"}; if both calls return ok, record ok, then call ui_hide(step_card_id).`,
    `Result values: use ok, degraded, error, or skipped_* for each item. Continue to the next item even if one fails.`,
    `[FINAL TWO STEPS — REQUIRED]\n(a) Call ui_show to display SelfCheckCard with props: { results: [{name:"文件读写",status:"ok/error",...},{name:"联网能力",...},{name:"结果通道",...}], overall:"ok/degraded/error" }. Infer overall from actual results: all ok → ok; any skipped → degraded; any error → error.\n(b) Call complete_startup_self_check with a summary (one sentence) and the results object.`,
  ].join('\n')
}
...
async function runTurn(input, label, msg = null) {
  ...
  try {
    ...
    if (isVoiceChannel(msg?.channel)) {
      ...
      directions.push('Voice language contract: always write the spoken answer in English, even when the user speaks Chinese. After the complete English answer, append a blank line, the exact marker `[中文翻译]`, and a faithful Simplified Chinese translation for the screen. Never put Chinese characters before the marker. The TTS engine can only speak the English portion.')
    ...
    const agentName = getConfig('agent_name') || '贾维斯'
    ...
    const geoMeta = getGeoWeatherMeta()
    ...
    const systemPrompt = buildSystemPrompt({
      ...
      currentCountryCode: geoMeta.countryCode,
      currentTimezone: geoMeta.timezone,
...
async function onTick() {
  ...
  try {
    ...
    if (hasMessages()) {
      ...
      const shouldRunIdleTurn = Boolean(
        state.startupSelfCheck?.active ||
        getAwakeningTicks() > 0 ||
        state.task ||
        getCustomIntervalMs() !== null ||
        shouldRunAutonomousTicks()
      )
      if (!shouldRunIdleTurn) {
        emitEvent('idle', {
          mode: 'monitor',
          nextCheckMs: IDLE_MONITOR_INTERVAL_MS,
...
function scheduleNextTick() {
  ...
  if (hasPendingUser) {
    ...
    interval = shouldRunAutonomousTicks() ? config.tickInterval : IDLE_MONITOR_INTERVAL_MS
    label = shouldRunAutonomousTicks() ? `${interval / 1000}s autonomous` : 'idle monitor'
  }
...
async function startConsciousnessLoop({ runImmediateTick = true } = {}) {
  ...
  loopStarted = true
  const skipStartupSelfCheck = shouldSkipStartupSelfCheck()
  ...
  if (skipStartupSelfCheck) {
    state.startupSelfCheck = {
      version: STARTUP_SELF_CHECK_VERSION,
      status: 'skipped',
      active: false,
      skipped_at: nowTimestamp(),
    }
    console.log('[system] Startup self-check skipped by environment')
  } else {
  ...
  if (runImmediateTick && !skipStartupSelfCheck) {
    await onTick()
  }
  scheduleNextTick()
}
...
async function main() {
  ...
  const apiPort = Number(process.env.JARVIS_PORT || process.env.JARVIS_PORT) || 3721
  startAPI(apiPort, {
    getStateSnapshot: () => ({
      action: state.action,
      task: state.task,
  ...
  const desktopMode = /^(1|true|yes|on)$/i.test(String(
    process.env.JARVIS_DESKTOP || process.env.JARVIS_DESKTOP || ''
  ).trim())
  if (desktopMode) {
    console.log('[TUI] Desktop mode detected; terminal TUI disabled')
  } else {
    startTUI('ID:000001')
  }
